...
client.on('messageDelete', message =>{ // ивент, когда удаляется любое сообщение с сервера https://discord.js.org/#/docs/main/stable/class/Client?scrollTo=e-guildMemberAdd
  ...
  client.channels.cache.get("866854842806042684").send(embed); // айди вашего канала с логами
})
...
client.on('guildMemberAdd', member =>{ // ивент, когда пользователь присоединяется к серверу https://discord.js.org/#/docs/main/stable/class/Client?scrollTo=e-guildMemberAdd
    ...
    Ты наш \`${client.guilds.get("833888165839765525").memberCount}\` участник! **`) // айди вашего сервера               !!!!!!!!!!
    ...
    .setColor('RANDOM')
    member.send(embed);
    client.channels.cache.get('866854842806042684').send(embed2) // айди вашего канала с логами
})
...
async function change() {
    let members = client.guilds.cache.get("833888165839765525").memberCount // сколько людей на сервере + указать айди своего сервера
    client.channels.cache.get("867639265571110932").setName(`На сервере: ${members}`); // свой айди войса
}

var interval = setInterval(function () { change(); }, 20000  ); // время обновления в миллисекундах

client.login('ODQ4ODQ3MTA5OTg5NTk3MTg0.YLSkeQ.ZQRUgXF_bkpqpejxpsvhXi3J9mI') // токен вашего бота
